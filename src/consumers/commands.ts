...
const USAGE = 'Usage: /outcome <new|criterion|list|status|verify|accept|reject|revise|abandon|export|delete> …'
      ...
      try {
        switch (command) {
          ...
          case 'criterion': {
            ...
            const built = specForCriterion(kind ?? '', args)
            if (built === undefined) {
          ...
          case 'export': {
            ...
            for (const contract of contracts.value) {
              ...
              out.push(
                ...
                `approve with /outcome export --approve ${preview.value.previewDigest} ${contract.id}`,
              )
            }
            return { kind: 'success', text: out.join('\n') }
          }
          case 'export-approve': {
            const digest = args.options.get('approve') ?? args.positionals[1]
            const id = contractId(args.positionals[2] ?? '')
            if (digest === undefined || digest === 'true') {
              return { kind: 'error', text: 'export --approve requires a preview digest' }
            }
            const result = await service.exportJsonl({ contractId: id, previewDigest: digest })
            if (!result.ok) return { kind: 'error', text: formatError(result.error) }
            return { kind: 'success', text: `Export approved: ${result.value.manifest.id}\n${result.value.content}` }
          }
          case 'delete': {
            const id = contractId(args.positionals[1] ?? '')
