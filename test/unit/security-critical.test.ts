...
/**
 * Same probe as in verification.test.ts: Windows without Developer Mode
 * cannot create symlinks (EPERM). We run a one-shot probe before the suite
 * and skip the symlink-only test when the OS won't allow it. The
 * production code (`realpath`-based containment) is still covered by the
 * lexical-escape assertions in other suites.
 */
let _canSymlink: boolean | undefined
async function probeSymlinks(): Promise<boolean> {
...
describe('active verifier paths', () => {
  ...
  it.skipIf(!(_canSymlink === true))('symlinked targets outside the workspace are rejected (unknown, never pass)', async () => {
